...
abstract class WbtyTable extends Table
{
	...
	public function load($keys = null, $reset = true)
	{
		...
		if (property_exists($this, 'checked_out_time') && property_exists($this, 'checked_out')) {
            $date = new Date($this->checked_out_time);
			$base = $date->toUnix();
            $current_date = new Date();
			if ($base > 0 && $current_date->toUnix() - $base > 120) {
				if ($this->checked_out == Api::getInstance()->getUser()->id) {
					$this->checkOut(Api::getInstance()->getUser()->id);
				} else {
					$this->checkIn();
				}
			}
