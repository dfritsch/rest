...
abstract class Table extends Object
{
	 ...
	 * @since  12.1
	 */
	private static $_includeSpaces = array();
	 ...
	 * classes see JTable::addIncludeSpace().
	 *
	...
	{
		// Search for the class file in the include namespaces.
		foreach (self::addIncludeSpace() as $namespace) {
			$tableClass = $namespace . $type;
		...
		if (!class_exists($tableClass)) {
			throw new \Exception('Table ('.$type.') not found', 500);
			return false;
		}

		// If a database object was passed in the configuration array use it, otherwise get the global one from Api.
		$db = isset($config['dbo']) ? $config['dbo'] : Api::getInstance()->getDbo();
	 ...
	 *
	 * @link    http://docs.joomla.org/JTable/addIncludeSpace
	 * @since   11.1
	 */
	public static function addIncludeSpace($path = null)
	{
		// If the internal paths have not been initialised, do so with the base table path.
		if (empty(self::$_includeSpaces))
		{
			self::$_includeSpaces = array('\\' . __NAMESPACE__ . '\\');
		}
		...
		if (!empty($path))
		{
			...
			foreach ($path as $dir)
			{
				...
				if (!in_array($dir, self::$_includeSpaces))
				{
					array_unshift(self::$_includeSpaces, $dir);
				}
			}
		}

		return self::$_includeSpaces;
	}
